      ...
      <el-table-column label="操作" width="150px" align="center">
        <template slot-scope="scope">
          ...
          <el-popover
           ...
           <el-button v-if="scope.row.status=='1'" slot="reference" type="success" size="mini"  @click="approve(scope.row.id)">通过</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页组件-->
...
export default {
  ...
  methods: {
    ...
    approve(id) {
      var sql = "update  car set `status`=2 where id=?";
      sql = sql.replace("?", id);
