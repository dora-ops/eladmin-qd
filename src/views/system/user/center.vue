            ...
            <ul class="user-info">
              ...
              <li>
               ...
               <li>
                ...
                <div class="user-right">{{ userInfo.sno }}</div>
              </li>
               <li>
                <svg-icon icon-class="email" /> 专业
                <div class="user-right">{{ userInfo.major }}</div>
              </li>
               <li>
                <svg-icon icon-class="email" /> 总学分
                <div class="user-right">{{ userInfo.total_score }}</div>
              </li>
              <li>
                <svg-icon icon-class="email" /> 个人学分
                <div class="user-right">{{ userInfo.person_score }}</div>
              </li>
              <li>
                <svg-icon icon-class="email" /> 个人能力
                <div class="user-right">{{ userInfo.a_score }}</div>
              </li>
              <li>
                <svg-icon icon-class="email" /> 文体活动
                <div class="user-right">{{ userInfo.w_score }}</div>
              </li>
               <li>
                <svg-icon icon-class="email" /> 品德基础分
                <div class="user-right">{{ userInfo.p_base_score }}</div>
              </li>
              <li>
                <svg-icon icon-class="email" /> 品德奖励分
                <div class="user-right">{{ userInfo.p_prize_score }}</div>
              </li>
              <li>
                <svg-icon icon-class="email" /> 品德扣减分
                <div class="user-right">{{ userInfo.p_prize_score }}</div>
              </li>
              <li>
...
export default {
  ...
  created() {
    store.dispatch("GetInfo").then(() => {
      ...
      sql = sql.replace("?", this.user.id);
      this.$http.post("action", { sql: sql }).then(item => {
        this.userInfo = item.data[0];
      });
    });
  },
  methods: {
